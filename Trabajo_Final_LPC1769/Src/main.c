...
// Definiciones ADC:
#define ADC_FREQ 100000 // Valor de la frecuencia de conversion del ADC en Hz
...
void Config_ADC(void) {
  ...
  // configuracion de la interrupcion global del ADC:
...
void Config_DMA_DAC(void) {
  ...
  ListCfg.DstAddr = (uint32_t) & (LPC_DAC->DACR);
  ListCfg.SrcAddr = (uint32_t)&DAC_Value;
  ListCfg.NextLLI = (uint32_t)&ListCfg;
  ListCfg.Control = (1 << 0) | (2 << 18) | (2 << 21);
  ...
  DMACfg.SrcMemAddr = (uint32_t)&DAC_Value;
  DMACfg.SrcConn = 0;
  DMACfg.DstConn = GPDMA_CONN_DAC;
  DMACfg.DstMemAddr = 0;
  DMACfg.TransferWidth = 0;
  DMACfg.DMALLI = (uint32_t)&ListCfg;
  GPDMA_Setup(&DMACfg);
  ...
  DAC_SetDMATimeOut(LPC_DAC, 27000);
